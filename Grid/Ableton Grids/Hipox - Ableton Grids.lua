...
@version 1.0.16
@changelog
    + fix: don't treat Reaper_Warp_Template_modified.als as an install dependency (it's generated by the workflow)
@links
...
local UI_MSG = {
    ...
    MODIFIED_ALS_MISSING = "No modified Ableton project found yet. Run Action 1, then Save in Ableton (do not rename the file), then retry.",
...
VerifyDependencies = function()
    ...
    if not path_exists(template_dir) then
        ...
        -- NOTE: Reaper_Warp_Template_modified.als is a generated workflow artifact.
        -- It is created/overwritten after running Action 1 (and saving in Ableton),
        -- so it should not block a fresh install.
        local grid_als = template_dir .. sep .. "Reaper_Warp_Template_modified.als"
        if not reaper.file_exists(grid_als) then
            table.insert(notes, "Reaper_Warp_Template_modified.als not found yet (OK): it is generated by Action 1 (after saving in Ableton) and is only needed for Action 2.")
        end
...
local function Action_ApplyAbletonBeatgridToSelection()
    ...
    if not reaper.file_exists(grid_als) then
        ...
        feedback_error(UI_MSG.MODIFIED_ALS_MISSING)
        if run_id then DebugOutcome(run_id, "ABORT: missing modified ALS (run Action 1 then save in Ableton)") end
        return
    end
